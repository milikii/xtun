...
- canonical smoke **271 组**全绿，新增 ACME 邮箱必填、失败回退服务状态、菜单升级重载、高级项行为、WARP 问答、prompt 写入目标、依赖阶段与 SNI/双栈/菜单用例，以及确认页前 443 闸门用例；smoke 默认把 `ss` 桩成「端口全空闲」，宿主机上装着 xtun 时向导用例不再被真实 443 监听拦下。

...
安装后的主菜单按六组组织：`1 获取节点`、`2 查看状态与诊断`、`3 修改节点`、`4 升级与维护`、`5 网络与可选功能`、`6 恢复与卸载`，另有 `7 升级脚本`（未安装时是同一动作的 `5`）。安装任务位于 `4 → 7`。菜单顶部只显示精简状态，不执行配置自检或 TLS 握手；完整体检使用 `2`、`xtun status` 或 `xtun diagnose`。

...
- 三项外部验收各自只缺一类输入：**受控域名**（G3；`acme-http` 走 HTTP-01，不再需要 DNS 令牌）、**操作者与三端设备**（G2/G4）、**云控制台/电源控制 + 自然时间**（G1/G5）。
...
| G4 实际使用 | ⏳ | Android 端节点 1/2/3 与 **ECH 变体**已初步可用；节点 4/5、IPv6/H3/WARP、Windows v2rayN 与 Debian NAS Docker 未测 | Windows 与 NAS 设备、一次可用时段 |

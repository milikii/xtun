...
3. 维护者追加需求：主菜单要有可直接选择的「升级脚本到最新版」入口，通过交互把脚本自身升级到最新版；并把声明版本从 `1.1.0` 提升到 `1.1.1`。
...
| `lib/cli/core.sh` | 主菜单新增一级「升级脚本到最新版」：已安装菜单 `7`、未安装菜单 `5`，都落到 `run_menu_choice fresh:5` → 现有 `xtun update-script`（下载 GitHub `main` 最新 bundle、校验、确认后安装，失败走同代恢复）。已安装的「升级与维护 → 更新脚本」保留。 |
...
- 真实终端上选主菜单「升级脚本到最新版」：能看到下载/版本对比/确认，升级后入口重新打开显示新版本；离线或 GitHub 不可达时有明确失败信息且不留半装状态。

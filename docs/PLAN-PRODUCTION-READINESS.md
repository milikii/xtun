...
> **历史计划，执行入口已更新（2026-09-14）：** 本文保留上一轮 T01–T12 设计和实施记录；下文“当前/本轮”、基线、CI 和生产状态均属于当时记录。最新代码与验收见[批次 B 报告](REPORT-2026-09-14-BATCH-B.md)，前序恢复见[接手报告](REPORT-2026-09-14-W04R-W05R.md)，当前入口为 [PLAN](PLAN.md)，施工顺序为[详细 W 计划](PLAN-UX-RELIABILITY.md)，发布与设备安排以[决策 D18–D30](DECISIONS-UX-RELIABILITY.md#d18)为准。下一独立代码批次 C：W08.2/8.3；G1 强制断电待验证。完整范围统一发布，设备验证后排，本次提交与 push 已授权。
...
> 本文曾是 [2026-09-12 计划](archive/PLAN-2026-09-12.md) 的详细施工依据；参数与历史实施细节继续保留，本轮行为决策以 [交互与可靠性决策](DECISIONS-UX-RELIABILITY.md) 为准。
...
| 113 条 smoke | [2026-09-12 PLAN](archive/PLAN-2026-09-12.md) 记录了全量通过；远端 smoke 步骤也成功 | 本轮未重新执行全套 smoke，不把历史记录写成本次实测 |
...
建议把后续 `1.2.0` 定位为生产就绪版本，先做候选版本验证；版本号以最终交付范围为准。[2026-09-12 PLAN](archive/PLAN-2026-09-12.md) §3.1 的 SNI 正确性修复进入 B，§3.2 的 PQ 观察放到 G；普通客户端兼容性进入 C/E，签名兼容性仍留在 G。
...
沿用 [2026-09-12 PLAN §2](archive/PLAN-2026-09-12.md) 的 CI 修法，并把核心接入作为同阶段工单 T01 的组成部分。不重复实现已完成的订阅删除、PNG 导出或 bundle 签名修复。T01 可拆成脚本整理、共享解析/下载、目标核心 CI 三个小提交，最后联合验收。
...
继续采用 [2026-09-12 PLAN](archive/PLAN-2026-09-12.md) 中已详细设计的 `xray tls ping` 探测方案：只解析有 SNI 的握手结果，完整输出才给结论，超时/缺核心/旧命令/未知格式为可解释的 WARN。解析依据更新为 v26.9.9 的 `tls/ping.go`；该文件与此前核对的 v26.3.27 文件内容相同，不把源码相同写成已跑过新核心的证明。latest CI 继续监测输出格式变化。[S12]

普通安装不会因为目标缺少 ML-DSA 准备条件而失败；`>3500` 的长度要求与 `X25519MLKEM768` 能力分别显示。保留现有检查的退出约定：参数错误 1，硬性检查失败 2，仅告警不导致失败。详细用例复用 [2026-09-12 PLAN §3](archive/PLAN-2026-09-12.md)，避免写两份互相漂移的解析规范。

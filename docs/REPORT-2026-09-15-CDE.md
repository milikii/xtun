...
9 月 17 日重新计算的清单逐字等于 9 月 15 日保存的 `runtime-manifest.txt`；当前运行/测试文件与留存 VPS 候选归档比对的 71 个文件全部相同。这证明文件与当时最终记录一致，不把旧日志解释成新提交的 CI。
...
- **VPS 当前状态：** 9 月 17 日旧 ControlMaster 已失效，初次批处理 SSH 认证失败；使用旧会话中用户提供的凭据重连后，只读核验通过。托管文件及服务 PID/状态与 9 月 15 日最终快照完全一致，当前无 pending；SSH 文件和 root 密码指纹均未变。当前核心为 `26.9.9 / 52a412d / amd64`。新快照和仅含比较结果的记录在私有 `20260917-resume/` 证据目录。
- **既有 nginx 差异：** VPS 实际二进制 `1.30.1`，dpkg `1.22.1-9+deb12u9`；保留该差异，不据此宣称系统包一致。

## 5. 交付与下一步

接手时尚无 C/D/E 提交或 CI run。本轮已提交并 push `27aa96772e10676b4c7757ab20321cba3a2912d6`，对应 [CI run 35165787041](https://github.com/milikii/xtun/actions/runs/35165787041)。本地运行内容与历史最终证据一致。

报告、静态/链接检查、提交和 push 已完成，VPS 实时 pending 与服务状态已复核。继续核对该候选 CI，重点是首次执行的官方 Docker 客户端及三系统安装矩阵。

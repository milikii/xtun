...
第三方目标需要按实际 SNI 与 target 检查，不能只凭机构类别判断适用。`check-sni` 在安装和改 SNI 时执行项目预检，涵盖 TLS/密钥组/ALPN、证书、HTTP、地址、耗时与后量子就绪度；这些项目包含协议条件和项目策略，不能一概称为 Xray 的 13 项硬性要求。

其中「后量子就绪度」是**观察项**，不是硬门槛：只有本机 openssl 在默认分组下协商到后量子混合组、且证书链总长度严格大于 3500 字节才 PASS，其它已测得情况一律 WARN。WARN 只增加告警计数，不产生 FAIL、不改变退出码，也不阻断安装；本机 openssl（<3.5）没有 ML-KEM 时协商不到混合组，这是事实陈述，不代表目标站不支持。

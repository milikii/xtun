...
批次 B 已完成六组任务菜单、返回/取消、维护影响预览、单节点链接/二维码取用和 H3 持久意图。首次公开 CI 暴露的夹具、执行身份、geo 依赖和新装日志权限问题已修复；最终代码提交 `6956e01` 的 CI 通过，包含干净 Debian 13 的完整安装/诊断。arm64 与测试 VPS amd64 的 smoke 均为 222/222，安装/菜单 PTY 仍为 68/9；VPS 已更新最终运行内容，SSH 与本机生产保持原状。下一独立代码批次为 C：W08.2/8.3 安装身份与可信同版本行为。
...
| CI 边界 | 最终代码 `6956e0183568071e94cc47ce205813a6391ff29c` 的 [34853691008](https://github.com/milikii/xtun/actions/runs/34853691008) 成功，ShellCheck/smoke/PTY 与 Debian 13 安装容器两个 job 均通过。首次失败与修复保留在 §4.1/4.2；push 下 latest-check 按条件跳过 |
...
本机 arm64 的 222/222 smoke 与 54 个 Shell 静态检查通过；新增两例也在 umask 077 下通过。低权限用例显式准备可遍历的夹具配置目录，避免调用者 umask 把问题遮在读取配置阶段。最终 v3 归档在测试 VPS 的 222/222 原生 smoke 通过；§5 的 20 项维护记录继续对应初始 B 归档，不改写成新运行内容已重跑全部场景。
...
最终公开代码提交为 `6956e0183568071e94cc47ce205813a6391ff29c`，CI [34853691008](https://github.com/milikii/xtun/actions/runs/34853691008) 完成并成功。干净 Debian 13 容器从安装到诊断、五节点 PNG/文档检查通过；smoke 与两套 PTY 的 CI 步骤也通过。latest-check 在 push 下未运行，五节点真实传输、迁移矩阵及真人验收仍按原计划补齐。此后的报告整理只改文档，不改变运行或测试内容。

最终 VPS 另执行真实 bundle 更新、同包 noop 和已安装入口 diagnose，三个命令均返回 0；安装摘要为 `df15775f…60f4349`，与 v3 候选一致。前后托管配置、state、节点文档/PNG、四服务 PID/状态、SSH 配置及 root 密码指纹一致，无 pending。Xray/nginx/HAProxy/SSH 的 PID 仍为 987684/987737/987710/524，全部 active/enabled。本机生产再核对 62 条记录及三服务状态，无变化。

最终 VPS 私有证据为 `ci-followup/vps-verified-evidence.tar.gz`，SHA256 `0dc999487bd821ab5f429856643b6dc4fa996220cd821c9d12c29198f1bb7dbf`；已校验并提取为 `ci-followup/vps-final/`。含前后快照、222 条回归、更新/noop/诊断日志和 SSH 比较结果；不提交凭据或原始部署日志。

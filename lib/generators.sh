...
# Ubuntu 24.04 的 nginx 1.24 不认独立的 `http2 on;` 指令（1.25.1 起才有），
# 只能写回 listen 行的老语法，否则 `nginx -t` 直接失败。
nginx_server_config() {
  ...
  if nginx_version_at_least 1.25.1; then
    listen_line="listen 127.0.0.1:${NGINX_TLS_PORT} ssl;
    http2 on;"
  else
...
# xtun 接管 /etc/nginx/nginx.conf：worker_rlimit_nofile / worker_connections 只能写在
# 主配置里。接管受状态键 NGINX_MAIN_MANAGED 控制，升级上来的旧节点默认不接管。
nginx_main_config_text() {
  cat <<EOF
...
# nginx 打包的 unit 一个 LimitNOFILE 都没写，worker 拿到的就是 systemd 的默认软限额
# 1024。而 nginx 在这套架构里是纯反代：一条客户端连接要占两个 fd（下游一个、到 xray
# 的上游一个），发行版默认的 worker_connections 768 光是跑满就要 1536 个——先撞
# EMFILE，再谈连接数上限。这里对齐 xray.service 里的 LimitNOFILE，也和网络优化里
# 按内存抬起来的 fs.file-max 是同一笔账。
# 只能走 drop-in：xtun 不接管 /etc/nginx/nginx.conf，worker_rlimit_nofile 写不进去。
nginx_limits_dropin_text() {
  cat <<'EOF'
